...
test("notify-send uses fixed argv and shell false", async () => { const calls: unknown[] = []; const child = { once: (_name: string, callback: (code?: number) => void) => { if (_name === "close") queueMicrotask(() => callback(0)); return child } } as never; await sendLinuxNotification("Title", "Body", (command, args, options) => { calls.push(command, [...args], options); return child }); assert.deepEqual(calls, ["notify-send", ["--app-name", "OpenCode", "--urgency", "normal", "Title", "Body"], { shell: false, stdio: "ignore" }]) })
